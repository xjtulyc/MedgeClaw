...
name: galaxy-bridge
description: >
  Galaxy tool discovery, intelligent recommendation, and execution — 8,000+
  bioinformatics tools from usegalaxy.org with multi-signal scoring across 7
  dimensions and workflow suggestions for 8 pipeline templates. Supports natural
  language search, remote execution via BioBlend, and offline demo mode.
  Triggers: galaxy, usegalaxy, tool shed, bioblend, run on galaxy, galaxy tool,
  galaxy workflow, NGS pipeline, bioinformatics, genomics, proteomics, metagenomics.
  触发关键词: Galaxy工具, 生物信息流水线.
version: 0.2.0
metadata:
  openclaw:
    emoji: "🌌"
