...
description: >
  ...
  Computes identity-by-state similarity, per-chromosome breakdown, and continental ancestry proportions
  from 23andMe data against a bundled PGP-1 reference genome.
  Input: 23andMe raw genotype file. Output: markdown report, IBS bar chart, ancestry pie, context gauge.
  Trigger keywords: genome comparison, IBS, identity by state, George Church, Corpasome, pairwise, admixture.
  基因组比较, 祖源估计.
version: 0.1.0
metadata:
  openclaw:
