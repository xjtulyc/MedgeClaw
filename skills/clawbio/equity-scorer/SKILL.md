...
name: equity-scorer
description: >
  Compute HEIM (Health Equity Index for Minorities) diversity and equity metrics from VCF or ancestry data.
  Generates heterozygosity analysis, pairwise FST, PCA visualisation, ancestry distribution,
  and a composite HEIM Equity Score (0-100) measuring population representation.
  Input: VCF file or ancestry CSV with population labels. Output: markdown report, PCA plot, ancestry bar chart,
  FST heatmap, heterozygosity comparison, HEIM score JSON.
  Trigger keywords: diversity scoring, HEIM equity, population representation, heterozygosity, FST, biobank diversity.
  群体公平性, 代表性评分.
version: 0.1.0
metadata:
  openclaw:
