...
name: gwas-prs
description: >
  Calculate polygenic risk scores (PRS) from DTC genetic data using the PGS Catalog.
  Computes dosage-weighted risk scores from 23andMe or AncestryDNA genotype files, estimates population percentiles,
  and assigns risk categories (low / average / elevated / high). Supports 6 curated scores and PGS Catalog API search.
  Input: 23andMe or AncestryDNA raw genotype file. Output: markdown report, CSV scores table, bell curve figure.
  Trigger keywords: PRS, polygenic risk score, polygenic score, genetic risk score, GWAS risk, multi-gene risk.
  多基因风险评分, 遗传风险.
version: 0.1.0
metadata:
  openclaw:
